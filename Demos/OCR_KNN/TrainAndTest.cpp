...
//Custom functions
int capturarImagen (cv::Mat image);
...
int main() {
    ...
    //cv::Mat matTestingNumbers = cv::imread("test3.png");            // read in the test numbers image

    cv::Mat matTestingNumbers;
    //    capturarImagen(matTestingNumbers);
    ...
    //Camera.set ( CV_CAP_PROP_FRAME_WIDTH, 2560 );
    ...
    Camera.grab();
    Camera.retrieve (matTestingNumbers);

    //cv::Mat edges;
    //cv::Canny(image,edges,10,200,3);

    //double secondsElapsed= double ( cv::getTickCount()-time_ ) /double ( cv::getTickFrequency() ); //time in second
    //cout<< secondsElapsed<<" FPS = "<< ( float ) ( ( float ) ( 1 ) /secondsElapsed ) <<endl;

    //cv::imshow("Bordes", edges);
    //cv::waitKey(0); 
    //    cv::imshow("Imagen Capturada",matTestingNumbers);
    //cv::waitKey(0);
    //cv::imwrite ("image.jpg",image);
    //cout<<"Image saved in image.jpg"<<endl;
